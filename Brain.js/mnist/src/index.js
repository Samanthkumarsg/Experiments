...
window.addEventListener(
	...
	() => {
		...
		// Clear canvas
		document.getElementById("sketchClearButton").addEventListener(
			"click",
			event => {
				let val = `Write more! its so fun guessing them :)`;
				document.querySelector("#PredictedValue").innerHTML = val;
				event.preventDefault();
				clearer();
			},
			false
		...
		document
			.getElementById("sketchRecogniseButton")
			.addEventListener("click", recognizeN, false);
	},
	false
); // end window.onLoad
	...
	fetch("/predict", {
		...
		.then(res => {
			res.json().then(result => {
				let obj = result.results;
				let el = obj.indexOf(Math.max.apply(null, obj));
				document.querySelector("#PredictedValue").innerHTML =
					"Gotcha !! Its " + el;
			});
		})
		.catch(err => {
